...
describe('Edit user spec', function () {
    ...
    it('GIVEN `User` with a role has been saved WHEN the user has been clicked THEN correct role should be displayed on the statistic panel',
        () => {
            ...
            return testUtils.clickOnSystemOpenUserWizard(webDriverHelper.browser).then(()=> {
                ...
                assert.equal(roles[1], appConst.roles.USERS_ADMINISTRATOR, '`Content Manager Administrator` role should be present on the panel')
            }).then(()=> {
    ...
    it('GIVEN existing user is opened WHEN display name has been changed THEN user should be searchable with the new display name',
        () => {
            return testUtils.selectUserAndOpen(testUser.displayName).then(()=>{
                ...
                return expect(userBrowsePanel.isExist(testUser.displayName)).to.eventually.be.true;
            })
        });
